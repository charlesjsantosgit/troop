...
func _simulate(dt: float) -> void:
	spool = move_toward(spool,
		input_throttle if driver else 0.0, 1.1 * dt)
	...
	var wash := spool * PROP_WASH_STEER \
		+ v.length_squared() * (95.0 if _in_water else 20.0)
...
func _build_body() -> void:
	...
	add_box(body, Vector3(0.5, 0.04, 0.3), Vector3(0, 0.24, -0.12), cage_mat)

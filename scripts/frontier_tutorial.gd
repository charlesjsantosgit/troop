...
const LESSONS := {
	"basics": [
		...
		["Buy a banana", "Nana's menu belongs to Nana. Choose the Banana tile, set quantity to 1, then Buy. Your backpack and Nana's stock update together. Press I after closing trade to inspect your backpack.", "earth_market", "buy", "banana"],
...
func _refresh_card() -> void:
	...
	_panel.visible = active and not complete and not controller.ui.visible \
		...
		and not (is_instance_valid(city) \
			and (city.is_in_city() or city.is_inside() or city.panel.visible))
	var info := summary()

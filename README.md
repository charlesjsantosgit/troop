...
-  **An infinite jungle, four biomes deep** — Emerald Rainforest, Bamboo Grove,
  Flooded Wetland, and Cloud Highland, with stronger rolling hills, macro lakes
  hundreds of metres across, snow-capped mountain ranges above the tree line,
  and seed-derived packed-dirt roads linking the useful vehicle destinations.
...
-  **Satellite minimap** — baked straight from the world generator like a
  ...
  with altitude, and shows every player with their name. It remains north-up
  on foot and turns heading-up only for the vehicle you are driving, while
  names and its frame stay upright. M resizes, [ ] zoom.
...
The `highspeed` variant drives a synthetic 1,000 mph path at altitude and checks
the predictive lead, visible terrain and collision coverage, bounded queues,
...
exercising the streaming-correctness checks. The correctness gate also requires
the full current stratos ring, a safe collision floor for descent and parked
ground vehicles, and bounded wilderness-vehicle population.

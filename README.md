...
- **Altitude view scaling**: the far plane grows with your elevation — 2.2 km
  on the jungle floor, a full **15 miles (24 km)** from the tallest peaks or
  on the wing. A fourth "stratos" LOD tier (6.1 km sectors, one draw each,
  water baked into the mesh) appears inside atmospheric haze, so the growth
  is seamless; shadows and SSAO step down when airborne since they are
  invisible from altitude, keeping the peak view at ~147 FPS on an M4.
- **Satellite minimap** (Xaero-inspired): true top-down tiles baked from the
  same deterministic generator the terrain uses — hillshaded ground colors,
  depth-tinted lakes, snowcaps — cached per zoom tier and filled in a few
  pixel rows per frame. Auto-zooms out as you climb, shows players and their
  names, plus Captain Peel. **M** cycles small/large/hidden, **[ ]** zoom;
  preferences persist.
- **Your name sticks**: the menu remembers the last name you played with
  instead of re-rolling a random monkey every launch.

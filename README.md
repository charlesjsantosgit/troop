...
-  **A complete Earth-like world, twelve biomes deep** — continents, islands,
  huge oceans, freshwater lakes, rainforest, bamboo, wetlands, highlands,
...
-  **A four-monkey Moon expedition** — board a physical rocket, watch Earth
  recede during a full three-minute outbound flight, land under 1.62 m/s²
  gravity in a pressure suit, refill oxygen at the ship, and buy Moon Cheese
  from a tiny lunar villager. The fiery two-minute return ends with an ocean
  splashdown on Earth.
...
- **The generated wilderness is now a circumnavigable planet.** TROOP's
  deterministic playable sphere is 98,304 m around at game scale. Longitude
  joins exactly, while crossing either pole reflects latitude, advances
...
- **Twelve coherent Earth-like biomes shape continents and islands.** Emerald
...
- **Roads form a planet-wide hierarchy.** Analytic highways and regional roads
  span the world, meet seamlessly at longitude and pole boundaries, and connect
  to the authored motor pool, airfield, dock, and launch site. Their crowned
...
- **X opens a smooth planetary atlas.** Drag or use WASD/arrow keys to pan,
  ...
  selectable beside it. Incremental sampling, a global 144-tile cap, and a
  compact globe atlas keep interaction bounded even at 4K.
- **The Moon is playable, not a sky prop.** E boards one of four
  ...
  exact 180-second outbound presentation progresses from animated ignition and
  atmosphere exit through star/planet/constellation/nebula/galaxy vistas, a
  shrinking Earth, camera pan, lunar approach, and touchdown. The 120-second
  return adds visible re-entry plasma and flames before an ocean splashdown.
- **Lunar survival has readable rules.** Arrival equips every crew member with
  a visible pressure suit, twin oxygen tanks, and an 18-slot space backpack.
  ...
  normal pack has 12. One is guaranteed in the northwest origin supply hut and
  more can appear in wilderness huts. The Crater & Curd villager sells stackable
  Moon Cheese for three authoritative bananas each, only when it fits in the pack.
- **Planet and expedition state are multiplayer-authoritative.** The server owns
  ...
  and late-join state. Admins can use `/moon [player]` or `/earth [player]` (also
...
monkeys are issued a pressure suit, space backpack, and full oxygen tank; travel
also removes a passenger safely from any active rocket manifest.
...
godot --headless --path . --quit-after 30000 --script res://tests/lunarexpeditiontest.gd
...
far-detail age, and stratos canopy geometry. Its 100 FPS timing gate is evaluated
only on a non-headless display reporting at least 99 Hz; other environments mark
the overall timing result `UNVERIFIED` and exit with status 2 while still
